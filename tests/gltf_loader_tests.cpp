...
TEST_CASE( "GLTFLoader File Loading", "[gltf][loader][file]" )
{
	...
	SECTION( "Extract real triangle mesh data from glTF" )
	{
		...
		REQUIRE( rootNodes[0]->meshCount() > 0 );
		const auto meshPtr = scene->getMesh( rootNodes[0]->getMeshHandle( 0 ) );
...
TEST_CASE( "GLTFLoader Transform Extraction", "[gltf][loader][transform]" )
{
	SECTION( "Extract TRS from glTF node with translation, rotation, scale" )
	{
		...
		// This is approximately 90 degrees (p/2 radians) around Z-axis
		REQUIRE( std::abs( transform.rotation.x ) < 0.001f );
		REQUIRE( std::abs( transform.rotation.y ) < 0.001f );
		REQUIRE( std::abs( transform.rotation.z - 1.5708f ) < 0.01f ); // p/2 � 1.5708
	}
	...
	SECTION( "Extract transform from complex matrix with rotation using Mat4 functionality" )
	{
		...
		// For 45� Y rotation: cos(45�)=sin(45�)=v2/2�0.7071068
		...
		// 45-degree rotation around Y should give ~45 degrees (p/4 � 0.785 radians) in Y component
		REQUIRE( std::abs( transform.rotation.x ) < 0.01f );
		REQUIRE( std::abs( transform.rotation.y - 0.785f ) < 0.01f ); // p/4 � 0.785
