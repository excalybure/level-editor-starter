...
export namespace math
{
	...
	inline float random() { return globalRandom.random(); }
	inline float random( float min, float max ) { return globalRandom.range( min, max ); }
	inline int randomInt( int min, int max ) { return globalRandom.range( min, max ); }
	inline bool randomBool( float probability = 0.5f ) { return globalRandom.chance( probability ); }
	inline Vec2<float> randomUnitCircle() { return globalRandom.unitCircle(); }
	inline Vec3<float> randomUnitSphere() { return globalRandom.unitSphere(); }
	inline Vec3<float> randomInsideSphere() { return globalRandom.insideSphere(); }
	...
	inline float perlinNoise( float x, float y ) { return SimpleNoise::perlinNoise( x, y ); }
	inline float fractalNoise( float x, float y, int octaves = 4 ) { return SimpleNoise::fractalNoise( x, y, octaves ); }
	inline float turbulence( float x, float y, int octaves = 4 ) { return SimpleNoise::turbulence( x, y, octaves ); }
}
