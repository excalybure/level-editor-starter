...
| **Primitive-aware ECS import** | **None** | **Convert SceneNode tree with primitive-based MeshRenderer** |
...
### 3. Mesh Extraction (Updated for Primitive-Based Design)
...
### 8. ECS Import (Updated for Primitive-Based Design)
- For each SceneNode:
...
### 9. GPU Buffer Creation (Updated for Primitive-Based Design)
- **NEW APPROACH**: Each primitive manages its own GPU resources.
- If Phase 2 requires runtime-ready meshes: allocate default/upload heap buffers per primitive.
- Provide lightweight struct with buffer views per primitive.
- **Key Benefit**: Optimized for modern rendering with per-primitive draw calls.
- If deferred: leave TODO + test only CPU data integrity per primitive.
