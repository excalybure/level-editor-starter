...
- [ ] Create `runtime.mesh_rendering_system` module
- [ ] Implement `MeshRenderingSystem : public systems::System`
- [ ] Query entities with `MeshRenderer` + `Transform` components
- [ ] Iterate primitives and call renderer for each

#### 1.2 Integrate with Existing Renderer
- [ ] Use existing `engine.renderer` for actual draw calls
- [ ] Support both immediate mode and mesh rendering
- [ ] Handle camera/view matrices from viewports
- [ ] Implement basic MVP matrix calculation

#### 1.3 GPU Resource Integration
- [ ] Use `MeshRenderer.gpuMesh` for GPU resources
- [ ] Call `PrimitiveGPU.bindForRendering()` for each primitive
- [ ] Handle materials via `MaterialGPU.bindToCommandList()`
- [ ] Implement primitive-level draw calls
...
**Acceptance Criteria**:
- [ ] System iterates all entities with MeshRenderer + Transform
- [ ] Calls renderer.drawIndexed() for each primitive 
- [ ] Uses GPU resources from MeshRenderer.gpuMesh
- [ ] Calculates correct MVP matrices from camera + transform
- [ ] Handles entities without GPU resources gracefully
...
- [ ] Integrate SceneEditor with main app
- [ ] Add MeshRenderingSystem to SystemManager
- [ ] Connect AssetManager + GPUResourceManager
- [ ] Update UI to include scene editor

#### 3.2 Asset-to-Render Pipeline
- [ ] Complete flow: glTF → Assets → ECS → GPU → Render
- [ ] Ensure proper resource lifetime management
- [ ] Handle async loading (if needed)
- [ ] Error handling throughout pipeline

#### 3.3 Viewport Integration
- [ ] Use viewport cameras for rendering
- [ ] Maintain grid rendering alongside meshes
- [ ] Preserve existing camera controls
- [ ] Handle multiple viewport rendering
...
**Acceptance Criteria**:
- [ ] Complete glTF-to-render pipeline working
- [ ] Can load and see a simple cube.gltf
- [ ] Meshes render with proper transformation
- [ ] Camera controls work with loaded meshes
- [ ] Grid still renders alongside meshes
