...
## 2025-01-10 — Data-Driven Material System: Root Signature Generation (T013)
**Summary:** Completed T013 (Generate root signature from material resources) implementing RootSignatureBuilder to create root signature specifications from MaterialDefinition. The builder aggregates resource bindings from material parameters (all treated as CBVs in this simplified version), validates no duplicate binding names, sorts bindings alphabetically for deterministic hashing, and assigns sequential slots. Phase 3 root signature synthesis now operational with 12 new assertions passing (121 total across material system).

**Atomic functionalities completed:**
- AF13: Root signature generation (T013) - Implemented RootSignatureBuilder::Build() converting material parameters to ResourceBinding list with CBV type, alphabetical sorting, and slot assignment

**Tests:** 26 test cases total, 121 assertions passing across material system. Commands: `unit_test_runner.exe "[T013]"` (3 tests, 12 assertions).

**Implementation Details:**
- **Root Signature Architecture**: RootSignatureSpec struct contains vector<ResourceBinding>, each binding has name/type/slot
- **Binding Types**: ResourceBindingType enum (CBV/SRV/UAV/Sampler) - currently only CBV used
- **Parameter Conversion**: AddParameterBindings() treats all material parameters as CBVs (simplified; full version would use shader reflection)
- **Duplicate Detection**: ValidateBindings() uses unordered_set to detect duplicate names, calls console::fatal if found
- **Deterministic Ordering**: SortBindings() sorts by name using operator<, ensuring consistent hashing for PSO cache (T014)
- **Slot Assignment**: AssignSlots() assigns sequential slots (0, 1, 2...) after sorting

**Files Modified:**
- src/graphics/material_system/root_signature_builder.h - RootSignatureSpec, ResourceBinding, ResourceBindingType, RootSignatureBuilder class
- src/graphics/material_system/root_signature_builder.cpp - Implementation with AddParameterBindings, ValidateBindings, SortBindings, AssignSlots
- tests/material_system_tests.cpp - 3 new test cases (single CBV, multiple sorted bindings, empty parameters)
- CMakeLists.txt - Added src/graphics/material_system/root_signature_builder.cpp to graphics library

**Notes:**
- Simplified implementation: treats all parameters as CBVs without shader reflection (full version T013.1 would parse DXIL)
- Deterministic ordering critical for T014 PSO caching (hash must be stable across runs)
- Duplicate binding validation provides defensive check (should never trigger with proper parameter validation)
- Ready for T014 (PSO building with root signature + render states + compiled shaders)

## 2025-01-10 — Data-Driven Material System: Shader Compilation Integration (T012)
**Summary:** Completed T012 (Compile shader with hierarchical defines) implementing MaterialShaderCompiler integration layer between material system and existing ShaderCompiler infrastructure. The compiler correctly converts material defines (unordered_map<string,string>) to shader compiler format (vector<string> in "KEY VALUE" form), sorts defines for deterministic compilation, and propagates shader compilation errors. Phase 3 shader compilation now operational with 11 new assertions passing (109 total across material system).

**Atomic functionalities completed:**
- AF12: Shader compilation with material defines (T012) - Implemented MaterialShaderCompiler::CompileWithDefines() converting DefineValidator's merged defines to shader compiler format, plus ConvertDefines() helper with deterministic sorting

**Tests:** 21 test cases total, 109 assertions passing across material system. Commands: `unit_test_runner.exe "[T012]"` (4 tests, 11 assertions).

**Implementation Details:**
- **Integration Architecture**: MaterialShaderCompiler static class with CompileWithDefines() facade over ShaderCompiler::CompileFromFile()
- **Define Conversion**: ConvertDefines() formats unordered_map as vector<string> in "KEY VALUE" format (e.g., {"FOO":"1"} → ["FOO 1"])
- **Deterministic Ordering**: std::sort() on result vector ensures consistent ordering for cache key generation (D3DCompile may produce non-deterministic bytecode timestamps, but define order must be stable)
- **Error Propagation**: Missing shader files throw from ShaderCompiler (via console::errorAndThrow), test confirms REQUIRE_THROWS catches exception
- **Edge Cases**: Empty defines map handled (zero-length vector to compiler), multiple defines tested (3+ simultaneous defines)

**Files Modified:**
- src/graphics/material_system/shader_compiler.h - New MaterialShaderCompiler class with CompileWithDefines() and ConvertDefines() methods
- src/graphics/material_system/shader_compiler.cpp - Implementation with unordered_map→vector<string> conversion and std::sort()
- shaders/test_material_defines.hlsl - Test shader using GLOBAL_DEFINE, PASS_DEFINE, MATERIAL_DEFINE in preprocessor conditionals
- tests/material_system_tests.cpp - 4 new test cases (merged hierarchical defines, empty defines, multiple defines, missing file)
- CMakeLists.txt - Added src/graphics/material_system/shader_compiler.cpp to graphics library

**Notes:**
- Deterministic ordering via std::sort() ensures consistent compilation caching in future (T014 will use defines hash for cache key)
- Existing ShaderCompiler already supports defines (vector<string> parameter), MaterialShaderCompiler is thin adapter
- BuildDefineString() in ShaderCompiler prepends "#define <each>\n" to shader source before D3DCompile
- Missing file error comes from ShaderCompiler::CompileFromFile() file existence check (throws with "Shader file not found: <path>")
- Ready for T013 (root signature generation from compiled shaders) and T014 (PSO building with cached bytecode)
...
**Tests:** 10 test cases total, 86 assertions passing across material system. Commands: `unit_test_runner.exe "[T009]"` (2 tests, 26 assertions) or `"[material-system]"` (all).
