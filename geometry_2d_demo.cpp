...
int main()
{
	...
	math::BoundingBox2D<float> box( math::Vec2<float>( 1.0f, 2.0f ), math::Vec2<float>( 5.0f, 6.0f ) );
	...
	std::cout << "   Contains (3,4): " << ( box.contains( math::Vec2<float>( 3.0f, 4.0f ) ) ? "yes" : "no" ) << "\n\n";

	// Point-in-shape tests
	std::cout << "2. Point-in-Shape Tests:\n";
	const math::Vec2<float> testPoint( 2.0f, 3.0f );
	std::cout << "   Test point: (" << testPoint.x << "," << testPoint.y << ")\n";

	// Circle test
	const math::Vec2<float> circleCenter( 2.5f, 3.5f );
	...
	// Rectangle test
	const math::Vec2<float> rectMin( 1.0f, 2.0f );
	const math::Vec2<float> rectMax( 3.0f, 4.0f );
	std::cout << "   In rectangle (" << rectMin.x << "," << rectMin.y << ") to (" << rectMax.x << "," << rectMax.y << "): "
			  << ( math::pointInRect( testPoint, rectMin, rectMax ) ? "yes" : "no" ) << "\n";

	// Triangle test
	const math::Vec2<float> triA( 0.0f, 0.0f );
	const math::Vec2<float> triB( 4.0f, 0.0f );
	const math::Vec2<float> triC( 2.0f, 4.0f );
	...
	math::Vec2<float> intersection;
	const bool intersects = math::lineLineIntersection(
		math::Vec2<float>( 0.0f, 0.0f ), math::Vec2<float>( 2.0f, 2.0f ), math::Vec2<float>( 0.0f, 2.0f ), math::Vec2<float>( 2.0f, 0.0f ), intersection );
	...
	const bool rayHitsCircle = math::rayCircleIntersection(
		math::Vec2<float>( -1.0f, 3.5f ), // Ray origin
		math::Vec2<float>( 1.0f, 0.0f ),  // Ray direction
	...
	const math::Vec2<float> point( 0.0f, 3.0f );
	const math::Vec2<float> lineStart( 1.0f, 1.0f );
	const math::Vec2<float> lineEnd( 3.0f, 1.0f );
